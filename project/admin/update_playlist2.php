...
if(isset($_POST['submit'])){
   ...
   $update_playlist = $conn->prepare("UPDATE `playlist2` SET title = ?, description = ?, status = ? WHERE id = ?");
   ...
   if(!empty($image)){
      if($image_size > 2000000){
         ...
         $update_image = $conn->prepare("UPDATE `playlist2` SET thumb = ? WHERE id = ?");
...
<section class="playlist-form">
   ...
   <?php
         $select_playlist = $conn->prepare("SELECT * FROM `playlist2` WHERE id = ?");
   ...
   <form action="" method="post" enctype="multipart/form-data">
      ...
      <div class="flex-btn">
         ...
         <a href="view_playlist2.php?get_id=<?= $playlist_id; ?>" class="option-btn">view playlist</a>
      </div>
   </form>
   <?php
      } 
