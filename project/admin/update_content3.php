...
if(isset($_POST['update'])){
   ...
   $playlist2 = $_POST['playlist3'];
   $playlist2 = filter_var($playlist, FILTER_SANITIZE_STRING);

   $update_content = $conn->prepare("UPDATE `content3` SET title = ?, description = ?, status = ? WHERE id = ?");
   $update_content->execute([$title, $description, $status, $video_id]);

   if(!empty($playlist)){
      $update_playlist = $conn->prepare("UPDATE `content3` SET playlist_id = ? WHERE id = ?");
   ...
   if(!empty($thumb)){
      if($thumb_size > 2000000){
         ...
         $update_thumb = $conn->prepare("UPDATE `content3` SET thumb = ? WHERE id = ?");
   ...
   if(!empty($video)){
      $update_video = $conn->prepare("UPDATE `content3` SET video = ? WHERE id = ?");
...
if(isset($_POST['delete_video'])){
   ...
   $delete_video_thumb = $conn->prepare("SELECT thumb FROM `content3` WHERE id = ? LIMIT 1");
   $delete_video_thumb->execute([$delete_id]);
   $fetch_thumb = $delete_video_thumb->fetch(PDO::FETCH_ASSOC);
   unlink('../uploaded_files/'.$fetch_thumb['thumb']);

   $delete_video = $conn->prepare("SELECT video FROM `content3` WHERE id = ? LIMIT 1");
...
<section class="video-form">
   ...
   <?php
      $select_videos = $conn->prepare("SELECT * FROM `content3` WHERE id = ? AND tutor_id = ?");
   ...
   <form action="" method="post" enctype="multipart/form-data">
      ...
      <select name="playlist" class="box">
         ...
         <?php
         $select_playlists = $conn->prepare("SELECT * FROM `playlist3` WHERE tutor_id = ?");
      ...
      <div class="flex-btn">
         <a href="view_content3.php?get_id=<?= $video_id; ?>" class="option-btn">view content</a>
   ...
   <?php
         ...
         echo '<p class="empty">video not found! <a href="add_content3.php" class="btn" style="margin-top: 1.5rem;">add videos</a></p>';
      }
   ?>
